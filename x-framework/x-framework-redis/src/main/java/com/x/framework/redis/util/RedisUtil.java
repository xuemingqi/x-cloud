package com.x.framework.redis.util;
...
public class RedisUtil {
    ...
    public <T> void set(String key, T value, long timeoutSec, TimeUnit timeUnit) {
        getRBucket(key).set(value, Duration.ofMillis(timeUnit.toMillis(timeoutSec)));
    }
    ...
    public <T> T getAndSet(String key, T value, long timeToLive, TimeUnit timeUnit) {
        RBucket<T> rBucket = getRBucket(key);
        return rBucket.getAndSet(value, Duration.ofMillis(timeUnit.toMillis(timeToLive)));
    }
    ...
    public void expired(String key, long timeoutSec, TimeUnit unit) {
        getRBucket(key).expire(Duration.ofMillis(unit.toMillis(timeoutSec)));
    }
    ...
    public <E> boolean sAddExpire(String key, long timeoutSec, E values) {
        RSet<E> rSet = getRSet(key);
        rSet.add(values);
        return getRSet(key).expire(Duration.ofSeconds(timeoutSec));
    }
    ...
    public <T> void lSet(String key, int index, T value, long timeoutSec) {
        ...
        rList.expire(Duration.ofSeconds(timeoutSec));
    }
    ...
    public <T> void lPushLeft(String key, T value, long timeoutSec) {
        ...
        rDeque.expire(Duration.ofSeconds(timeoutSec));
    }
     ...
     * @param unit   计数时间单位
     */
    public boolean tryAcquire(String key, long counts, long unit) {
        RRateLimiter limiter = getRRateLimiter(key);
        limiter.trySetRate(RateType.OVERALL, counts, Duration.ofSeconds(unit), Duration.ofDays(1));
    ...
    public long publish(String topic, Object message) {
        return getRTopic(topic).publish(message);
    }
     ...
     */
    public <T> int subscribe(String topic, Class<T> messageClass, MessageListener<? extends T> listener) {
        return getRTopic(topic).addListener(messageClass, listener);
    }
     ...
     * 发布订阅主题
     */
    private RTopic getRTopic(String topic) {
