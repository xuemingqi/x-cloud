...
public class JwtUtil {
    ...
    public String createToken(UserInfo user) {
        ...
        return JWT.create()
                ...
                .withClaim(user.getUserId(), JsonUtil.toJsonStr(user))
    ...
    public String getAudience(String token) {
        ...
        try {
            audience = JWT.decode(token).getAudience().getFirst();
        } catch (Exception e) {
            log.error("获取签发对象失败,error:{}", ExceptionUtils.getStackTrace(e));
        }
        return audience;
    }
     ...
     * @return 用户信息
     */
    private UserInfo getClaimByAudience(String token, String audience) {
        String str = JWT.decode(token).getClaim(audience).asString();
        return JsonUtil.jsonToBean(str, UserInfo.class);
    }
